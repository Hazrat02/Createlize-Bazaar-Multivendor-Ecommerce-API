      ...
      <div class="row align-items-center">
        <div class="col-md-6">
          <div class="title mb-30">
            <h2>{{ t('edit_delivery_type') }}</h2>
            <p class="text-sm text-muted">{{ t('update_delivery_type_details') }}</p>
          </div>
        </div>
        <div class="col-md-6">
...
const form = useForm({
  ...
  key: props.deliveryType.key || '',
  sort_order: props.deliveryType.sort_order ?? 0,
  is_active: !!props.deliveryType.is_active,
})

function submit() {
  form.put(`/admin/deliveries/${props.deliveryType.id}`)
}
</script>
