						...
						<div class="col-md-6 sticky-sidebar-wrapper">
							<div class="product-gallery pg-vertical sticky-sidebar"
								...
								<div
									...
									<figure class="product-image" v-for="image in productGallery" :key="image.image">
										<img :src="image.image" :data-zoom-image="image.zoom" :alt="product.name"
											width="800" height="900">
									</figure>
								</div>
								<div class="product-thumbs-wrap">
									<div class="product-thumbs">
										<div class="product-thumb" v-for="(image, index) in productGallery"
											:key="image.thumb" :class="{ active: index === 0 }">
											<img :src="image.thumb" alt="product thumbnail" width="109" height="122">
										</div>
									</div>
									<button class="thumb-up disabled"><i class="fas fa-chevron-left"></i></button>
									<button class="thumb-down disabled"><i class="fas fa-chevron-right"></i></button>
								</div>
								<div class="product-label-group" v-if="product.labels && product.labels.length">
									<label v-for="label in product.labels" :key="label.text" :class="label.class">{{
										label.text
									}}</label>
								</div>
							</div>
						...
						<div class="col-md-6">
							<div class="product-details">
								...
								<div class="product-price">
									<ins v-if="product.oldPrice" class="new-price">{{ formatPrice(product.price)
										}}</ins>
									<del v-if="product.oldPrice" class="old-price">{{ formatPrice(product.oldPrice)
										}}</del>
								...
								<div class="product-form product-qty">
									<div class="product-form-group">
										...
										<button class="btn-product btn-cart text-normal ls-normal font-weight-semi-bold"
											@click.prevent="addToCart(product, quantity)"><i
					...
					<section class="pt-3 mt-10">
						...
						<div class="owl-carousel owl-theme owl-nav-full row cols-2 cols-md-3 cols-lg-4"
							...
							'responsive': {
								...
								'992': {
									...
									'nav': true
													...
													<div class="product" v-for="item in relatedProducts" :key="item.id">
							<figure class="product-media">
								<NuxtLink :to="`/product/${item.slug}`">
									<img :src="item.image" :alt="item.name" width="280" height="315">
								</NuxtLink>
								<div class="product-label-group" v-if="item.labels && item.labels.length">
									<label v-for="label in item.labels" :key="label.text" :class="label.class">{{ label.text }}</label>
								</div>
							...
							<div class="product-details">
								...
								<div class="product-price">
									<ins v-if="item.oldPrice" class="new-price">{{ formatPrice(item.price) }}</ins>
									<del v-if="item.oldPrice" class="old-price">{{ formatPrice(item.oldPrice) }}</del>
									<span v-else class="price">{{ formatPrice(item.price) }}</span>
								</div>
								<div class="ratings-container">
									<div class="ratings-full">
										<span class="ratings" :style="{ width: `${item.ratingPercent}%` }"></span>
										<span class="tooltiptext tooltip-top"></span>
									</div>
									<a href="#" class="rating-reviews">( <span class="review-count">{{ item.reviewCount }}</span>
										reviews )</a>
								</div>
							</div>
						</div>
...
const { products, getProductBySlug, defaultGallery, formatPrice } = useProducts()
...
const productGallery = computed(() => defaultGallery)
const relatedProducts = computed(() =>
  products.value.filter((item) => item.id !== product.value.id).slice(0, 4)
)
definePageMeta({
  layout: 'riode'
})
