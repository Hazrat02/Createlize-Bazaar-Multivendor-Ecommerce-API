...
class CouponController extends Controller
{
    ...
    public function validateCoupon(Request $request)
    {
        ...
        $user = $request->user() ?? \App\Models\User::query()->where('email','guest@createlize.org')->first(); // for pre-login checks if you seed a guest
        if (!$user) {
            // fallback: create an in-memory user-like object is not allowed; require auth for strict per-user usage
